...
class SettingsRepository(private val context: Context) {
    ...
    suspend fun updateCaptureModeSettings(block: (CaptureModeSettings) -> CaptureModeSettings) {
        context.dataStore.edit { prefs ->
            ...
            prefs[Keys.VIDEO_CAPTURE_ENABLED] = updated.videoCaptureEnabled.toString()
    ...
    private fun Preferences.toAllSettings(): AllSettings {
        ...
        val watermark = WatermarkSettings(
            ...
            position = this[Keys.WATERMARK_POSITION]?.let {
                runCatching { com.charles.photobooth.template.WatermarkPosition.valueOf(it) }
                    .getOrDefault(com.charles.photobooth.template.WatermarkPosition.BOTTOM_RIGHT)
            } ?: com.charles.photobooth.template.WatermarkPosition.BOTTOM_RIGHT,
        ...
        val captureMode = CaptureModeSettings(
            ...
            boothIntervalSeconds = (this[Keys.BOOTH_INTERVAL] ?: "3").toIntOrNull() ?: 3,
            boothPhotoCount = (this[Keys.BOOTH_PHOTO_COUNT] ?: "4").toIntOrNull() ?: 4,
            gifModeEnabled = (this[Keys.GIF_MODE_ENABLED] ?: "false").toBooleanStrictOrNull() ?: false,
            videoCaptureEnabled = (this[Keys.VIDEO_CAPTURE_ENABLED] ?: "false").toBooleanStrictOrNull() ?: false,
        ...
        val defaultImmichUrl = if (BuildConfig.WEDDING_MODE) BuildConfig.WEDDING_IMMICH_URL else ""
        val defaultImmichToken = if (BuildConfig.WEDDING_MODE) BuildConfig.WEDDING_IMMICH_API_TOKEN else ""
        val defaultImmichAlbumId = if (BuildConfig.WEDDING_MODE) BuildConfig.WEDDING_IMMICH_ALBUM_ID else ""
        val defaultAlbumSync = if (BuildConfig.WEDDING_MODE) defaultImmichAlbumId.isNotBlank() else false

        val savedImmichToken = (this[Keys.IMMICH_API_TOKEN] ?: "").ifBlank { getSecureString(SecureKeys.IMMICH_API_TOKEN) }

        val upload = UploadSettings(
            autoUploadEnabled = (this[Keys.UPLOAD_AUTO_ENABLED] ?: if (BuildConfig.WEDDING_MODE) "true" else "false").toBooleanStrictOrNull() ?: BuildConfig.WEDDING_MODE,
            useAnonymousHost = (this[Keys.UPLOAD_USE_ANON] ?: if (BuildConfig.WEDDING_MODE) "false" else "true").toBooleanStrictOrNull() ?: !BuildConfig.WEDDING_MODE,
            immichBaseUrl = (this[Keys.IMMICH_BASE_URL] ?: defaultImmichUrl).ifBlank { defaultImmichUrl },
            immichApiToken = savedImmichToken.ifBlank { defaultImmichToken },
            immichAlbumSyncEnabled = (this[Keys.IMMICH_ALBUM_SYNC_ENABLED] ?: defaultAlbumSync.toString()).toBooleanStrictOrNull() ?: defaultAlbumSync,
            immichAlbumId = (this[Keys.IMMICH_ALBUM_ID] ?: defaultImmichAlbumId).ifBlank { defaultImmichAlbumId },
        )
        ...
        val savedSmsPassword = (this[Keys.SMS_PASSWORD] ?: "").ifBlank { getSecureString(SecureKeys.SMS_PASSWORD) }
        val sms = SmsGatewaySettings(
            baseUrl = this[Keys.SMS_BASE_URL] ?: "",
            username = this[Keys.SMS_USERNAME] ?: "",
            password = savedSmsPassword,
            useCloudServer = (this[Keys.SMS_USE_CLOUD] ?: "false").toBooleanStrictOrNull() ?: false,
        )

        val defaultSmtpHost = if (BuildConfig.WEDDING_MODE) BuildConfig.WEDDING_SMTP_HOST else ""
        val defaultSmtpPort = if (BuildConfig.WEDDING_MODE) BuildConfig.WEDDING_SMTP_PORT else SmtpSettings().port
        val defaultSmtpUseTls = if (BuildConfig.WEDDING_MODE) BuildConfig.WEDDING_SMTP_USE_TLS else true
        val defaultSmtpUsername = if (BuildConfig.WEDDING_MODE) BuildConfig.WEDDING_SMTP_USERNAME else ""
        val defaultSmtpPassword = if (BuildConfig.WEDDING_MODE) BuildConfig.WEDDING_SMTP_PASSWORD else ""
        val defaultSmtpFromAddress = if (BuildConfig.WEDDING_MODE) BuildConfig.WEDDING_SMTP_FROM_ADDRESS else ""
        val defaultSmtpFromName = if (BuildConfig.WEDDING_MODE) BuildConfig.WEDDING_SMTP_FROM_NAME else ""

        val savedSmtpPassword = (this[Keys.SMTP_PASSWORD] ?: "").ifBlank { getSecureString(SecureKeys.SMTP_PASSWORD) }
        val smtp = SmtpSettings(
            host = (this[Keys.SMTP_HOST] ?: defaultSmtpHost).ifBlank { defaultSmtpHost },
            port = this[Keys.SMTP_PORT] ?: defaultSmtpPort,
            useSslTls = (this[Keys.SMTP_USE_SSL] ?: defaultSmtpUseTls.toString()).toBooleanStrictOrNull() ?: defaultSmtpUseTls,
            username = (this[Keys.SMTP_USERNAME] ?: defaultSmtpUsername).ifBlank { defaultSmtpUsername },
            password = savedSmtpPassword.ifBlank { defaultSmtpPassword },
            fromAddress = (this[Keys.SMTP_FROM_ADDRESS] ?: defaultSmtpFromAddress).ifBlank { defaultSmtpFromAddress },
            fromName = (this[Keys.SMTP_FROM_NAME] ?: defaultSmtpFromName).ifBlank { defaultSmtpFromName },
