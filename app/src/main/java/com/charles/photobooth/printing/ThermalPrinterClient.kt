...
// Extra blank paper (at DOTS_PER_MM resolution) baked directly into the raster image so
// there's always a reliable amount of blank paper below the printed content to tear on, in
// addition to whatever the printer does with the ESC d feed command below. The ESC d "feed n
// lines" command's actual line height is not documented for this printer's custom YK protocol,
// so it can't be trusted alone to produce a consistent physical feed distance - baking the
// margin into the image itself guarantees it regardless of how the printer interprets ESC d.
// Half an inch, not a full inch: a full inch printed too long a tear-off tail.
private val EXTRA_TEAR_OFF_PX = (DOTS_PER_MM * 12.7f).roundToInt()
private const val BOTTOM_FEED_MARGIN_PX = 100
private const val POST_PRINT_FEED_LINES = 64
private const val POST_PRINT_FEED_COMMANDS = 1
// Conservative (slow) estimate of physical print speed for this class of printer, used only as
// a floor on how long we keep the socket open after sending the raster data - see the comment
// where this is used. Erring slow just means we wait a little longer than strictly necessary;
// erring fast is what caused prints to get cut off before.
private const val PRINT_SPEED_MM_PER_SEC = 15f
private const val MIN_PRINT_WAIT_MS = 3_000
private const val MAX_PRINT_WAIT_MS = 25_000
...
class ThermalPrinterClient(
    ...
    companion object {
        ...
        private const val YK_DENSITY = 15
    ...
    suspend fun print(bitmap: Bitmap): Result<Unit> = withContext(Dispatchers.IO) {
        runCatching {
            if (!com.yk.common.YkDataPacket.isLibraryLoaded) {
                throw IllegalStateException("Thermal printer is not supported on this 32-bit device architecture.")
            }
            ...
            val paperHeightMm = prepared.height / DOTS_PER_MM
            ...
            try {
                ...
                // The printer's ACK/status byte (read above) can arrive well before it has
                // physically finished feeding and printing every row - the Bluetooth transfer
                // is much faster than the print head. Closing the socket right after that early
                // ACK can truncate the tail of the print, which is exactly the bottom of the
                // image: the footer text and the tear-off margin. Wait out a generous minimum
                // print duration, regardless of how early the ACK arrived, before moving on.
                val estimatedPrintMs = (paperHeightMm / PRINT_SPEED_MM_PER_SEC * 1000)
                    .roundToInt()
                    .coerceIn(MIN_PRINT_WAIT_MS, MAX_PRINT_WAIT_MS)
                Log.d(TAG, "Waiting ${estimatedPrintMs}ms for ${paperHeightMm}mm of physical print to finish")
                Thread.sleep(estimatedPrintMs.toLong())
    ...
    private fun appendFooterAndBottomMargin(image: Bitmap): Bitmap {
        ...
        val output = Bitmap.createBitmap(
            image.width,
            image.height + footerHeight + BOTTOM_FEED_MARGIN_PX + EXTRA_TEAR_OFF_PX,
    ...
    private fun ditherForThermal(src: Bitmap): Bitmap {
        ...
        for (i in pixels.indices) {
            ...
            // Thermal paper prints noticeably lighter than the dithered bitmap suggests, so bias
            // harder toward black than a "correct" contrast/brightness adjustment would: boost
            // contrast more and shift everything darker before thresholding.
            lum[i] = ((luma - 128f) * 1.35f + 128f - 24f).coerceIn(0f, 255f)
        }
